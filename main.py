...
if selected_page=="Predict":
    ...
    padding_top = 5
    padding_bottom = 2
    style = f"background-color: grey; height: 60px; width: {width}px; border-radius: 5px; padding-top: {padding_top}px; padding-bottom: {padding_bottom}px; margin-left: 150px; text-align: center; margin-top:-50px;"

    label_style = "font-weight: bold; color: white; font-size: {font_size}px;"

    label_style = label_style.format(font_size=font_size)
    st.markdown(f"<span style='color: blue; font-size: 18px;'><p style='margin-top:30px;'><strong>Sleep Report:</strong></p></span><div style='{style}'><label style='{label_style}'>{pred_label}</label></div>", unsafe_allow_html=True)
